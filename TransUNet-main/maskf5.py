...
def sem_seg_postprocess(result, img_size, output_height, output_widht):
    result = result[:, : img_size[0], : img_size[1]].expand(1, -1, -1, -1)
    result = F.interpolate(
        result, size=(output_height, output_widht), mode='bilinear', align_corners=False
    )[0]
    return result
        ...
        with tqdm(total=epoch_step_val, desc=f'Epoch {epoch_num + 1}/{epoch}', postfix=dict, mininterval=0.3, colour='cyan') as pbar:
            ...
            for i, sampled in enumerate(valloader):
                ...
                with torch.no_grad():
                    ...
                    f1 += vf1
                    giou += vgiou
                pbar.set_postfix(**{'miou': miou / (i + 1), 'acc': acc/(i+1), 'oa': oa/(i+1), 'f1': f1/(i+1), 'giou': giou/(i+1)})
                pbar.update(1)
        lr_scheduler.step()
