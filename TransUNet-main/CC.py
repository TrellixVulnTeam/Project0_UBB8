...
def InPlaceABNSync(channel):

    return(
        nn.Sequential(nn.BatchNorm2d(channel),
        nn.ReLU()
    ))
...
class RCCAModule(nn.Module):
    def __init__(self, in_channels, out_channels, num_classes):
        ...
        self.convb = nn.Sequential(nn.Conv2d(inter_channels, inter_channels, 3, padding=1, bias=False),
                                   InPlaceABNSync(inter_channels))
        self.bb = nn.Conv2d(512, num_classes, kernel_size=1, stride=1, padding=0, bias=True)
        self.bottleneck = nn.Sequential(
...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes, criterion, recurrence):
        ...
        self.dsnn = nn.Conv2d(512, num_classes, kernel_size=1, stride=1, padding=0, bias=True)
    ...
    def forward(self, x, labels=None):
        ...
        x = self.layer3(x)
        x_dsn = self.dsn(x)
        x_dsn = self.dsnn(x_dsn)
        x_d = F.interpolate(x_dsn,size=(512,512),mode='bilinear',align_corners=True)
        x = self.layer4(x)

        x = self.head(x, self.recurrence)
        # x = torch.cat([x_dsn, x], 1)
        # x = self.bb(x)
