...
"""
import glob
from dataProcess1 import get_dataloader
import torch
import cv2
import numpy as np
import os
...
def test(model_path_effi7, model_path_resnest, output_dir, test_loader, addNDVI):
    ...
    for image, image_stretch, image_path in test_loader:
        ...
        for i in range(output.shape[0]):
            ...
            pred = np.uint8(pred)
            save_path = os.path.join(output_dir, 'a', image_path[i][-10:])
                                     # .replace('.tif', '.png'))
            print(save_path)
...
if __name__ == "__main__":
    ...
    model_path_resnest = '../Model/unetplusplus_dpn131_Sentinel2_SoftCE_dice.pth'
    ...
    output_dir = r'E:\data\classify\result'
    test_image_paths = glob.glob(r'E:\data\classify\test\*.tif')
    addNDVI = False
    # batch_size = 16
    num_workers = 8
    batch_size = 4
