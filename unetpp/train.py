...
import warnings
import time
from dataProcess1 import get_dataloader, cal_val_iou, split_train_val,cal_iou
...
def train(EPOCHES, BATCH_SIZE, train_image_paths, train_label_paths, 
    ...
    train_loader = get_dataloader(train_image_paths, train_label_paths, 
                                  "train", addNDVI, BATCH_SIZE, shuffle=True, num_workers=6)
    valid_loader = get_dataloader(val_image_paths, val_label_paths, 
                                  "val", addNDVI, BATCH_SIZE, shuffle=False, num_workers=6)
    ...
    header = r'Epoch/EpochNum | TrainLoss | ValLoss | ValidmIoU | Time(m)'
    raw_line = r'{:5d}/{:8d} | {:9.3f} | {:9.3f} | {:9.3f} | {:9.2f}'
    print(header)
...
#    scaler = GradScaler()
    ...
    for epoch in range(1, EPOCHES+1):
        ...
        for batch_index, data in enumerate(train_loader):
            ...
            # target = target.swapaxes(2, 3).swapaxes(2, 1)
...
#            with autocast(): #need pytorch>1.6
        ...
        # 输出进程
        print(raw_line.format(epoch, EPOCHES, np.array(losses).mean(), np.array(vallosses).mean(),
                              np.mean(val_iou), 
...
#    scaler = GradScaler()
    ...
    for epoch in range(1, EPOCHES+1):
        ...
        for batch_index, (image, target) in enumerate(train_loader):
            image = image.swapaxes(2, 1).swapaxes(2, 3)
            image, target = image.to(DEVICE), target.to(DEVICE)
            # 在反向传播前要手动将梯度清零
            optimizer.zero_grad()
#            # 使用autocast半精度加速训练,前向过程(model + loss)开启autocast
...
if __name__ == '__main__':
    EPOCHES = 100
    BATCH_SIZE = 2
    image_paths = glob.glob(r'E:\data\YAMATO\train\*.tif')
    label_paths = glob.glob(r'E:\data\YAMATO\labelgray\*.tif')
    ...
    model_path = "../Model/unetplusplus_dpn131_POTS2"
    if(upsample):
